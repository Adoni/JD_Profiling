...
def construct_all_data():
    ...
    fout=open(RAW_DATA_DIR+'prlabel2trainset/all_train.data','w')
    uid_output=open(RAW_DATA_DIR+'prlabel2trainset/all_train_uids.data','w')
...
def construct_train_set(attribute,training_count):
    ...
    for index,user in enumerate(collection.find()):
        ...
        for f,value in user['mentions_1'].items():
            if f in labeled_features:
                label_distributed[0]*=labeled_features[f][0]*value/labeled_feature_distribute[f]
                label_distributed[1]*=labeled_features[f][1]*value/labeled_feature_distribute[f]
    ...
    fout=open(RAW_DATA_DIR+'prlabel2trainset/%s_train.data'%attribute,'w')
    uid_output=open(RAW_DATA_DIR+'prlabel2trainset/%s_train_uids.data'%attribute,'w')
...
def construct_test_set(attribute):
    ...
    bar=progress_bar(collection.count())
    fout=open(RAW_DATA_DIR+'prlabel2trainset/%s_test.data'%attribute,'w')
    uid_output=open(RAW_DATA_DIR+'prlabel2trainset/%s_test_uids.data'%attribute,'w')
    for index,user in enumerate(collection.find()):
        features=dict(Counter(user['products']))
        for m in user['mentions']:
            features[m]=user['mentions'][m]
